...
interface GroupDAO {
    ...
    @Query("SELECT  studentId, FIO, COUNT(CASE WHEN attendance_table.statys = 'Отсутствовал' THEN 1 ELSE NULL END) AS summ FROM student_table LEFT JOIN attendance_table ON student_table.id = attendance_table.studentId GROUP BY student_table.FIO")
